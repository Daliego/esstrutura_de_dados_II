...
class Arvore {
	...
	bool arvoreBalanceada(ArvoreNo<T> *root){

			if(root == nullptr){
				return true;
			}

			int alturaEsquerda = alturaArvore(root->right);
			int alturaDireita = alturaArvore(root->left);

			if(abs(alturaEsquerda - alturaDireita) > 1){
				return false;
			}

			bool balanceamentoEsquerdo = arvoreBalanceada(root->left);
			bool balanceamentoDireito = arvoreBalanceada(root->right);

			return balanceamentoEsquerdo && balanceamentoDireito;
		}
...
main(){
	...
	//a->percusoExtensao();
	// cout << a->arvoreBalanceada(a->getRoot()) << endl;
	//cout << "" << endl;
	//a->preorder(a->getRoot());
